...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($forum_id)) {
        ...
        $fetchQuery = "SELECT forum_file_upload FROM forums WHERE forum_id = '$forum_id' AND user_id = '$user_id'";
        $fetchResult = mysqli_query($con, $fetchQuery);

        if (mysqli_num_rows($fetchResult) > 0) {
            $forumData = mysqli_fetch_assoc($fetchResult);

            $delete_query = "DELETE FROM forums WHERE forum_id = '$forum_id' AND user_id = '$user_id'";
