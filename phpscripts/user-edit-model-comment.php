...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['model_comment_id']) && isset($_POST['comment_text']) && isset($_POST['logged_in_user_id'])) {
        $loggedInUserId = mysqli_real_escape_string($con, $_POST['logged_in_user_id']);
        $commentId = mysqli_real_escape_string($con, $_POST['model_comment_id']);
        $newCommentText = mysqli_real_escape_string($con, $_POST['comment_text']);

        $edit_comment_query = "UPDATE model_comments SET comment = '$newCommentText' WHERE model_comment_id = '$commentId' AND user_id = '$loggedInUserId'";
